...
--*/
{
    ...
    HidDevice->HidDevice = CreateFile (DevicePath,
								   ...
								   NULL,   // Open device as non-overlapped so we can get data
...
BOOL HID_Write(UCHAR *WriteBuf, DWORD WriteBufSize)
{
	BOOL status = 0;
	DWORD WriteCount = 1025, WrittenCount = 0;
	UCHAR *Temp_Buf;

	Temp_Buf = (unsigned char *)malloc(WriteCount);
	if (Temp_Buf == NULL)
	{
		printf("Returned null pointer.. HID_Write error = %d\n", 80); //Error = 80 is just an indicative value 
		return 80;
	}
	...
	if (WriteBufSize > 1024)
	{
		printf("size greater than 1022...send correct size\n");
		return 1;
	}
	...
	{
		Temp_Buf[i + 1] = WriteBuf[i];
	}

	status = WriteFile(HidDevHandle,
		Temp_Buf,
		WriteCount,
		&WrittenCount,
		NULL);
	...
	if (status == 0)
		printf("Write status = %d, HidDevHandle = 0x%x, bytesToWrite = %d,  bytesWritten = %d, last error = %d\n", status, HidDevHandle, WriteCount, WrittenCount, GetLastError());
...
BOOL HID_GetReport(UCHAR *ReportData)
{
	BOOL status = 0;

	status = HidD_GetInputReport(HidDevice->HidDevice,
			HidDevice->InputReportBuffer,
			HidDevice->Caps.InputReportByteLength);

	if (status == 0)
		printf("GetInputReport status = %d, HidDevHandle = 0x%x, bytesToRead = %d, last error = %d\n", status, HidDevice->HidDevice, HidDevice->Caps.InputReportByteLength, GetLastError());

	for (int i = 0; i < HidDevice->InputReportBuffer[1]; i++)
		ReportData[i] = HidDevice->InputReportBuffer[i + 1] ;

	return status;
}

BOOL HID_SetReport(UCHAR *ReportData, INT count)
{
	BOOL status = 0;
	UINT localCounter = 300;

	if (count > 510)
		return 0;

	HidDevice->OutputReportBuffer[0] = 0;
	HidDevice->OutputReportBuffer[1] = count;
	for (int i = 0; i < count; i++)
	HidDevice->OutputReportBuffer[i+2] = ReportData[i];

	status = HidD_SetOutputReport(HidDevice->HidDevice,
		HidDevice->OutputReportBuffer,
		HidDevice->Caps.OutputReportByteLength);

	if (status == 0)
		printf("GetOutputReport status = %d, HidDevHandle = 0x%x, bytesToWrite = %d, last error = %d\n", status, HidDevice->HidDevice, HidDevice->Caps.OutputReportByteLength, GetLastError());

	return status;
}

void ISP_I2C_Write(INT regAddr, INT regValue)
{
	UCHAR outBuf[64], inBuf[64];

	outBuf[0] = 0xBB;

	outBuf[1] = (UCHAR)((regAddr >> 8) & 0xFF);
	outBuf[2] = (UCHAR)((regAddr >> 0) & 0xFF);

	outBuf[3] = (UCHAR)(regValue & 0xFF);

	HID_SetReport(outBuf, 4);
	Sleep(10);
	HID_GetReport(inBuf);
	if (inBuf[0] != 2)
	{
		printf("ISP I2C Write failed, error code = %d\n", inBuf[2]);
	}
